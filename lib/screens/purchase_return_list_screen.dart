...
class _PurchaseReturnListScreenState extends State<PurchaseReturnListScreen> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error loading purchase returns: $e')),
        );
      }
    }
  }

  List<Invoice> _getPurchaseReturns() {
    try {
      final returns = (invoiceBox.values
          .where((invoice) =>
              invoice.isReturn == true &&
              (invoice.originalInvoiceNumber?.startsWith('PINV') == true ||
                  invoice.invoiceNumber.startsWith('PRET')))
          .toList()
  ...
  Future<void> _navigateToReturn(Invoice returnInvoice) async {
    // Find the original purchase invoice if it exists
          ...
          child: Padding(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Search by invoice #, supplier...',
      ...
      body: _isLoading
          ...
          : ValueListenableBuilder(
              ...
              builder: (context, Box<Invoice> box, _) {
                final returns = _getPurchaseReturns();
                if (returns.isEmpty) {
                  return const Center(
                    child: Text('No purchase returns found'),
                  );
                }
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            Text(
                              'Supplier: ${returnInvoice.partyName}',
                              maxLines: 1,
